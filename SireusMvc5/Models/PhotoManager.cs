...
namespace SireusMvc5.Models
{
    ...
    public class PhotoManager
    {
                ...
                var connection = new SqlConnection(ConfigurationManager.ConnectionStrings["Personal"].ConnectionString))
            {
                using (var command = new SqlCommand("GetPhotos", connection))
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while ((reader.Read()))
                        {
                            var temp = new Photo(Convert.ToInt32(reader["PhotoID"]), Convert.ToInt32(reader["AlbumID"]),
                                Convert.ToString(reader["Caption"]));
                            //list.Add(temp);
                ...
                var connection = new SqlConnection(ConfigurationManager.ConnectionStrings["Personal"].ConnectionString))
            {
                using (var command = new SqlCommand("GetAlbums", connection))
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while ((reader.Read()))
                        {
                            var temp = new Album(Convert.ToInt32(reader["AlbumID"]),
                                Convert.ToInt32(reader["NumberOfPhotos"]), Convert.ToString(reader["Caption"]),
        ...
        private static Size CalculateDimensions(Size oldSize, int targetSize)
        {
            ...
            {
                newSize.Width =
                    Convert.ToInt32((oldSize.Width*Convert.ToSingle((targetSize/Convert.ToSingle(oldSize.Height)))));
            ...
            else
            {
                ...
                newSize.Height =
                    Convert.ToInt32((oldSize.Height*Convert.ToSingle((targetSize/Convert.ToSingle(oldSize.Width)))));
            }
            return newSize;
        }
